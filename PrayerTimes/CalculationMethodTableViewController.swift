...
class CalculationMethodTableViewController: UITableViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if indexPath.section == 0 {
            ...
            NSUserDefaults.standardUserDefaults().setInteger(Int(prayerMethodeNames[indexPath.row].methodeNumber), forKey: "MethodNumber")
            tableView.reloadData()
        } else if indexPath.section == 1 {
            calcMethodNumber.asrMethodChecked = indexPath.row
            NSUserDefaults.standardUserDefaults().setInteger(Int(asrPrayerMethodNames[indexPath.row].methodeNumber), forKey: "AsrMethodNumber")
            tableView.reloadData()
        }
    }
